...
class LoteriaApp:
    def __init__(self, root):
        ...
        self.label_imagen = tk.Label(self.main_frame, bg="#fef9f5", width=200, height=270)
        ...
        self.label_count = tk.Label(self.main_frame, text="Cartas mostradas: 0", font=self.font_label, fg=self.primary_color, bg="#fef9f5")
        self.label_count.pack(pady=2)
        ...
        self.btn_pausa = self.crear_boton("Pausar", self.toggle_pausa, 1)
        self.btn_reiniciar = self.crear_boton("Reiniciar", self.reiniciar_juego, 2)
        self.btn_verificar = self.crear_boton("Verificar", self.verificar_carton, 3)
    ...
    def iniciar_juego(self):
        ...
        self.label_count.config(text="Cartas mostradas: 0")
        self.mostrar_carta()

    def mostrar_carta(self):
        if not self.jugando or self.pausado:
            return
        ...
        self.label_count.config(text=f"Cartas mostradas: {self.indice + 1}")
        self.agregar_miniatura(carta)
        driver.say(carta)
        driver.runAndWait()
        self.indice += 1
        self.after_id = self.root.after(2000, self.mostrar_carta)

    def agregar_miniatura(self, carta):
        file = f"{CARTAS.index(carta)+1}_{remove_accents(carta).replace(' ', '_')}.jpg"
        path = os.path.join(CARDS_FOLDER, file)
        if os.path.exists(path):
            img = Image.open(path).resize((60, 80), Image.LANCZOS)
            photo_img = ImageTk.PhotoImage(img)
            self.thumbnails_images.append(photo_img)
            label = tk.Label(self.inner_frame, image=photo_img, bg="#fef9f5")
            label.pack(side=tk.RIGHT, padx=4)
            self.inner_frame.update_idletasks()
            self.canvas_cartas.config(scrollregion=self.canvas_cartas.bbox("all"))
            self.canvas_cartas.xview_moveto(1)  # Scroll to the rightmost

    def limpiar_miniaturas(self):
        for widget in self.inner_frame.winfo_children():
            widget.destroy()
        self.thumbnails_images.clear()
        self.canvas_cartas.xview_moveto(0)
